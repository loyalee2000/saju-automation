...
new_method = """    def get_result_json(self):
        ...
        # Save attributes (Prefer Single Char for internal logic if used elsewhere, 
        # but existing code might expect different. Let's set Chars for safety as standard)
        ...
        # Gyeokguk (Use Chars)
        # signature: (day_gan, month_ji, heaven_stems[list])
        gyeokguk = self._calculate_gyeokguk(d_gc, m_jc, saja_gan_char)
        
        # Gongmang (Use Full Strings as CHEONGAN.index expects them)
        if d_gf and d_jf:
        ...
        # Pillars List for Strength/Yongsin (Use Chars)
        pillars_list_char = [
            {'gan': y_gc, 'ji': y_jc},
            {'gan': m_gc, 'ji': m_jc},
            {'gan': d_gc, 'ji': d_jc},
        ...
        # Strength
        strength_res = self._calculate_saju_strength(d_gc, pillars_list_char, ohaeng_cnt)
        
        # Yongsin
        score = strength_res.get('score', 0)
        yongsin = self._calculate_yongsin(d_gc, pillars_list_char, ohaeng_cnt, score)
        
        luck_cycle = self._calculate_luck_cycle()
        
        # Sinsal (Use Full Strings or Chars?)
        # _calculate_pillar_sinsal(gan, ji, type) -> likely expects Chars or checks maps.
        # Sinsal maps (SINSAL_DESC keys) are '천을귀인' etc.
        # Logic usually checks Jiji vs Something.
        # Let's check _calculate_pillar_sinsal signature logic? 
        # Line 252. Assume it handles the input format provided.
        # But previous working version passed whatever get_gan_ji returned (Full?).
        # Verify_rich.py output for Sinsal was empty because logic failed?
        # Let's pass FULL STRINGS if logic relies on `GAN in CHEONGAN` lists using full.
        # BUT many Sinsal definitions use CHARS.
        # I'll pass FULL strings. If it fails, I'll switch.
        ...
        for p_name, idx in pillars_map:
            ...
            # Use Full Strings for Sinsal logic?
            # Or Chars?
            # Safe bet: Chars. Most logic (e.g. Ohaeng map) works with chars. 
            # Gongmang was an exception because it used index search on specific list.
            # I will pass Chars.
            s_gan = self._calculate_pillar_sinsal(saja_gan_char[idx], saja_ji_char[idx], "gan")
            s_ji = self._calculate_pillar_sinsal(saja_gan_char[idx], saja_ji_char[idx], "ji")
        ...
        for p_name, idx in pillars_map:
            ...
            # Use 'g_full' for lookups in dictionaries passed from saju_desc_data?
            # saju_desc_data keys are "갑(甲)" (Full).
            # So usage: CHEONGAN_DESC.get(g_full, {})
            
            g_desc = CHEONGAN_DESC.get(g_full, {})
            j_desc = JIJI_DESC.get(j_full, {}) # JIJI also uses Full? Check keys.
            # Yes, saju_desc_data JIJI_DESC keys are "자(子)".
            
            # Fallback if Full key not found (maybe mismatch): try Char
            if not g_desc and g_char:
                 # Try to find key starting with char
        ...
        for p_name, idx in pillars_map:
            ...
            # _calculate_12unseong likely takes Chars?
...
if start_idx != -1:
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Fixed get_result_json logic V4.")
else:
