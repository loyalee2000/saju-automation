...
# ========== FIX 1: Daewoon Rounding (round -> ceil) ==========
# Line ~690: daewoon_num = int(round(diff_days / 3))

old_daewoon = "daewoon_num = int(round(diff_days / 3))"
new_daewoon = """import math
        daewoon_num = int(math.ceil(diff_days / 3))"""

content = content.replace(old_daewoon, new_daewoon)
print("FIX 1: Applied Daewoon ceil fix.")


# ========== FIX 2: Sinsal Logic in _calculate_pillar_sinsal (Line 252) ==========
# Issues:
#   - 천주귀인 map: 기 -> 오 (WRONG), should be 기 -> 유
#   - 천의성: Only checks Month Ji, should also check Year Ji vs target
#   - 현침살: '미' missing in Ji check
#   - 양인살: Checking d_gan only, but for 기(己) and Hour Ji=미(未), it should trigger

# Find and replace the entire _calculate_pillar_sinsal method
start_marker = "    def _calculate_pillar_sinsal(self, gan, ji, pillar_type):"
end_marker_search = "\n    def _calculate_sinsal("

start_idx = content.find(start_marker)
end_idx = content.find(end_marker_search, start_idx + 100)

if start_idx == -1:
    print("ERROR: Could not find _calculate_pillar_sinsal.")
    exit(1)
...
content = content[:start_idx] + new_sinsal_method + content[end_idx:]
print("FIX 2: Replaced _calculate_pillar_sinsal with corrected logic.")


# ========== FIX 3: Remove duplicate sinsal method if exists ==========
# Check for another _calculate_pillar_sinsal definition and remove it
second_def = content.find("    def _calculate_pillar_sinsal(self, gan, ji, pillar_type):", start_idx + 100)
if second_def != -1:
    # Find its end
    second_end = content.find("\n    def ", second_def + 100)
    if second_end != -1:
        content = content[:second_def] + content[second_end:]
        print("FIX 3: Removed duplicate _calculate_pillar_sinsal.")
    else:
        print("WARNING: Could not find end of duplicate method.")
else:
    print("FIX 3: No duplicate method found.")
...
print("\nAll fixes applied. Restart the app to test.")
