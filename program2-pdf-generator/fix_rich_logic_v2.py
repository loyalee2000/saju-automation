...
# Replace get_result_json again.
# To be robust, let's look for "def get_result_json(self):" until next "def ".

new_method = """    def get_result_json(self):
        # 1) Calculate basic pillars
        # get_gan_ji returns { 'year': "갑(甲)자(子)", ... }
        pillars_data = self.get_gan_ji()
        
        # Ohaeng (Pass the dict directly)
        ohaeng_cnt = self.analyze_ohaeng(pillars_data)
        
        # Extract Chars for Sinsal/Desc lookups
        # Format is "갑(甲)자(子)" (G(H)J(H)) length 8
        # Gan at 0, Ji at 4
        ...
        for p_name, idx in pillars_map:
            ...
            # Use original text from pillars_data for full display
            full_text = pillars_data[p_name] # "갑(甲)술(戌)"
            # Extract just "갑(甲)" and "술(戌)" parts if needed or use chars.
            # User wants: "gan": "갑(甲)", "ji": "술(戌)".
            # We can slice: gan_full = text[:4], ji_full = text[4:]
        ...
        # 3. Twelve Unseong (Placeholder/Simple Logic)
        # 12 Unseong is typically Day Gan vs Branch.
        # Let's add basic mapping if we can, or empty dict if risky.
        # User requested it. Let's output empty dict but with keys to match structure if possible.
        twelve_unseong_res = {
             "year": {"stage": "-", "desc": {}},
             "month": {"stage": "-", "desc": {}},
             "day": {"stage": "-", "desc": {}},
...
if start_idx != -1:
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Fixed get_result_json logic V2.")
else:
